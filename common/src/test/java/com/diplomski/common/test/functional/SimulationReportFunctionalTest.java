...
public class SimulationReportFunctionalTest {
	...
	private @NonNull ISimulationService simulationService;
	...
	public void generateSimulationReport() {
		SimulationReport simulationReport = simulationService.getSimulation(
		...
		assertNotNull(simulationReport);

		assertNotEquals(
				"Player always wins", 
				simulationReport.getWinCount(), 
				SIMULATION_COUNT
		);
		
		assertNotEquals(
				"Enemy always wins",
				simulationReport.getLossCount(), 
				SIMULATION_COUNT
		);
		
		assertNotEquals(
				"All battles are a draw",
				simulationReport.getDrawCount(),
				SIMULATION_COUNT
		);
	}
}
